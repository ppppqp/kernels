...
def tl_reduce_sum(A, BLOCK_N: int, BLOCK_M: int):
    ...
    with T.Kernel(T.ceildiv(N, BLOCK_N), threads=256) as pid_n:
        ...
        for m_blk_id in T.Serial(M // BLOCK_M):
            # we don't need to specify range here?
            T.copy(A[n_idx, m_blk_id * BLOCK_M], A_local)
            T.reduce_sum(A_local, B_local, dim=1, clear=False) # accumulate the result to B_local
        
        T.copy(B_local, B[n_idx : n_idx + BLOCK_N])
