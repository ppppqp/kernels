...
def tl_mul_relu_bcast(A, B, BLOCK_N: int, BLOCK_M: int):
    ...
    with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=256) as (pid_n, pid_m):
        ...
        T.copy(B[m_idx : m_idx + BLOCK_M], B_local)
        for i, j in T.Parallel(BLOCK_N, BLOCK_M):
            C_local[i, j] = A_local[i, j] * B_local[j]
            C_local[i, j] = T.if_then_else(C_local[i, j] > 0, C_local[i, j], T.float16(0))
        
        T.copy(C_local, C[n_idx : n_idx + BLOCK_N, m_idx : m_idx + BLOCK_M])
    return C
...
def tl_mul_relu_bwd(A, B, dC, BLOCK_N: int, BLOCK_M: int):
    ...
    with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=256) as (pid_n, pid_m):
        ...
        T.copy(A[n_idx : n_idx + BLOCK_N, m_idx : m_idx + BLOCK_M], A_local)
        T.copy(B[m_idx : m_idx + BLOCK_M], B_local)
        T.copy(dC[n_idx : n_idx + BLOCK_N, m_idx : m_idx + BLOCK_M], dC_local)
        for i, j in T.Parallel(BLOCK_N, BLOCK_M):
            dA_local[i, j] = dC_local[i, j] * B_local[j]
            dA_local[i, j] = T.if_then_else(A_local[i, j] * B_local[j] > 0, dA_local[i, j], T.float16(0))
        
        T.copy(dA_local, dA[n_idx : n_idx + BLOCK_N, m_idx : m_idx + BLOCK_M])
