...
def tl_scalar_flash_attn(Q, K, V, BLOCK_B: int, BLOCK_S: int):
    ...
    with T.Kernel(B // BLOCK_B, threads=256) as pid_b:
        ...
        running_max = T.alloc_fragment([BLOCK_B], dtype)
        running_sum = T.alloc_fragment([BLOCK_B], dtype)

        T.fill(running_max, -T.infinity(dtype))
        T.clear(running_sum)

        # The first loop uses online softmax state to compute the final normalization.
        for s_blk_id in T.Serial(S // BLOCK_S):
            ...
            for i in T.Parallel(BLOCK_B):
                new_max = T.if_then_else(
                    running_max[i] > cur_max_QK[i], running_max[i], cur_max_QK[i]
                )
                running_sum[i] = (
                    running_sum[i] * T.exp2((running_max[i] - new_max) * log2_e)
                    + cur_sum_exp_QK[i] * T.exp2((cur_max_QK[i] - new_max) * log2_e)
                )
                running_max[i] = new_max

        # The second loop uses the finalized online softmax state to get the final output.
        ...
        for s_blk_id in T.Serial(S // BLOCK_S):
            ...
            for i, j in T.Parallel(BLOCK_B, BLOCK_S):
                O[pid_b * BLOCK_B + i, s_blk_id * BLOCK_S + j] = (
                    T.exp2((Q_local[i, j] * K_local[i, j] - running_max[i]) * log2_e)
                    / running_sum[i]
                    * V_local[i, j]
                )
