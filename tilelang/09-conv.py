...
def tl_conv1d_naive(X, K, BLOCK_N: int, BLOCK_L: int):
    ...
    with T.Kernel(N // BLOCK_N, L // BLOCK_L) as (bx, by):
        X_local = T.alloc_shared ([BLOCK_N, BLOCK_L + KL], dtype)
        K_local = T.alloc_fragment([KL], dtype)
        O_local = T.alloc_fragment([BLOCK_N, BLOCK_L], dtype)
        T.clear(O_local)
        T.copy(K, K_local)
        T.copy(X[bx * BLOCK_N, by * BLOCK_L], X_local)
        for x, y in T.Parallel(BLOCK_N, BLOCK_L):
            for k in T.Serial(KL):
                O_local[x, y] += K_local[k] * X_local[x, y + k]

        T.copy(O_local, O[bx * BLOCK_N, by * BLOCK_L])
    return O
...
def tl_conv1d_multi_outchannel(X, K, BLOCK_N: int, BLOCK_L: int):
    ...
    with T.Kernel(N // BLOCK_N, L // BLOCK_L) as (bx, by):
        ...
        T.copy(K, K_local)
        T.copy(X[bx * BLOCK_N, by * BLOCK_L], X_local)
        for x, y, f in T.Parallel(BLOCK_N, BLOCK_L, F):
            for k in T.Serial(KL):
                O_local[x, y, f] += K_local[k, f].astype(accum_dtype) * X_local[x, y + k].astype(accum_dtype)

        T.copy(O_local, O[bx * BLOCK_N, by * BLOCK_L, 0])
    return O
...
def tl_conv1d_im2col(X, K, BLOCK_N: int, BLOCK_L: int):
    ...
    # K_shared = T.alloc_shared((KL, F), dype)
    # O_local = T.alloc_fragment((BLOCK_N * BLOCK_L, F), accum_dtype)

    with T.Kernel(N // BLOCK_N, L // BLOCK_L) as (bx, by):
        X_local = T.alloc_fragment ([BLOCK_N, BLOCK_L, KL], dtype)
        K_local = T.alloc_fragment([KL, F], dtype) # has to be shared, somehow matters for layout inference
        O_local = T.alloc_fragment([BLOCK_N * BLOCK_L, F], accum_dtype)
        # T.clear(O_local)
        T.copy(K, K_local)
        # T.copy(X[bx * BLOCK_N, by * BLOCK_L], X_local)
        # prepare x:
        for x, y, k in T.Parallel(BLOCK_N, BLOCK_L, KL):
            X_local[x, y, k] = T.if_then_else(by * BLOCK_L + y + k < L, X[bx * BLOCK_N + x, by * BLOCK_L + y + k], 0)
        X_reshaped = T.reshape(X_local, (BLOCK_N * BLOCK_L, KL))
        T.gemm(X_reshaped, K_local, O_local, clear_accum=True)
        O_reshaped = T.reshape(O_local, (BLOCK_N, BLOCK_L, F))
        T.copy(O_reshaped, O[bx * BLOCK_N, by * BLOCK_L, 0])
