...
def tl_dequant_matmul(A, B, BLOCK_M: int, BLOCK_N: int, BLOCK_K: int):
    ...
    with T.Kernel(M // BLOCK_M, N // BLOCK_N, threads=128) as (bx, by):
        A_local = T.alloc_shared([BLOCK_M, BLOCK_K], A_dtype)
        B_local = T.alloc_shared([BLOCK_K, BLOCK_N // 2], B_storage_dtype)
        ...
        for k in T.Pipelined(K // BLOCK_K, num_stages=3):
            ...
            T.copy(B[k * BLOCK_K, by * BLOCK_N // 2], B_local)
            for i, j in T.Parallel(BLOCK_K, BLOCK_N // 2):
                # can not assign to parameter. Feel like this should be fixed?
                # B_low = T.cast(B_local[i, j] & 0x0F, A_dtype) - 8.0
                # B_high = T.cast((B_local[i, j] >> 4) & 0x0F, A_dtype) - 8.0
                B_dequantized[i, 2 * j] = T.cast(B_local[i, j] & 0x0F, A_dtype) - 8.0
                B_dequantized[i, 2 * j + 1] = T.cast((B_local[i, j] >> 4) & 0x0F, A_dtype) - 8.0
            T.gemm(A_local, B_dequantized, C_local)    
