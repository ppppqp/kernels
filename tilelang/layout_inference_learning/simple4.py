...
def plan_then_buffer_layout():
    ...
    def main(
        ...
        with T.Kernel(1, threads=64):
            ...
            # it through a projected two-variable access:
            #
            #   fragment[group * 2 + row_in_group, 0]
            #
            # ComputeLoopLayoutFromBuffer substitutes that access into the
            # fragment's thread expression:
            #
            #   thread = (group * 2 + row_in_group) * 16
            #          = group * 32 + row_in_group * 16
            #
            # Only threads {0, 16, 32, 48} are used. This sparse mapping cannot
            # be represented by InverseAffineIterMap without an additional
            # divisibility guard, so current TileLang fails while lowering it.
